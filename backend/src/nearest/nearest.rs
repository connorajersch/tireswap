...
impl NearestStationFinder {
    ...
    pub fn find_k_nearest(&self, lat: f64, lon: f64, k: usize) -> Vec<StationWithDistance> {
        ...
        let nearest = self.kdtree.nearest_n::<SquaredEuclidean>(&[lon, lat], candidates);
...
mod tests {
    ...
    fn test_haversine_distance() {
        ...
        let distance = NearestStationFinder::haversine_distance(ny_lat, ny_lon, london_lat, london_lon);
    ...
    fn test_haversine_same_point() {
        ...
        assert!(distance < 0.01, "Distance should be nearly zero but was {}", distance);
    }
}
